...
ggplot(evalCntPerPkg , aes(x = "" , y = freq)) + geom_boxplot() + labs(y="Number of Evals", x="Packages", title="Evals distribution in the packages - boxplot")

##outliers
outlyingVals <- boxplot(evalCntPerPkg$freq, plot = F)$out
outlyingRows <- filter(evalCntPerPkg, freq %in% outlyingVals)
##top outliers
knitr::kable(head(arrange(outlyingRows, desc(freq)) , 5) , col.names = c("Package" , "Number of evals"))
##bottom outliers
knitr::kable(head(arrange(outlyingRows, freq) , 5) , col.names = c("Package" , "Number of evals"))

#plotting without the outliers
evalCntPerPkgMod <- evalCntPerPkg[ !(evalCntPerPkg$freq %in% outlyingVals), ]
ggplot(evalCntPerPkgMod , aes(x = "" , y = freq)) + geom_boxplot() + labs(y="Number of Evals", x="Packages", title="Evals distribution in the packages - boxplot (outliers removed)")
